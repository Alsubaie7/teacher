...
const Print = {
  ...
  timetable() {
    ...
    const times   = st.periods || [];
    const teacher = DB.teacher() || {};
    const main    = teacher.subject || '';
    ...
    /* يحترم عدد الحصص المعتمد — كان ثابتاً على ٨ فيطبع صفوفاً ملغاة */
    const periods = Array.from({ length: st.periodCount || 8 }, (_, i) => i + 1);

    const hdr = days.map(d => `<th class="d">${d.l}</th>`).join('');
    const rows = periods.map(p => {
      const t = times.find(x => x.p === p);
      const cells = days.map(d => {
        const e = sched.find(x => x.day === d.k && x.period === p);
        const c = e ? classes.find(x => x.id === e.classId) : null;
        if (!c) return '<td class="e"></td>';
        const sub = e.subject || c.subject || '';
        return `<td class="f" style="--c:${clsColor[c.id]}">
          <span class="n">${_esc(c.name)}</span>
          ${sub && sub !== main ? `<span class="s">${_esc(sub)}</span>` : ''}
        </td>`;
      }).join('');
      return `<tr>
        <td class="p"><b>${p}</b>${t ? `<span>${t.s}<br>${t.e}</span>` : ''}</td>
        ${cells}
      </tr>`;
    }).join('');

    const legend = classes.map(c =>
      `<span class="lg"><i style="background:${clsColor[c.id]}"></i>${_esc(c.name)}</span>`).join('');
    const today = new Date().toLocaleDateString('ar-SA');

    const markup = `<!DOCTYPE html><html dir="rtl" lang="ar"><head>
      <meta charset="UTF-8"><title>جدول الحصص — ${_esc(teacher.name || '')}</title>
      <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;900&display=swap" rel="stylesheet">
      <style>
        @page{size:A4 landscape;margin:10mm}
        *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
        body{font-family:'Tajawal',Arial,sans-serif;direction:rtl;color:#1f2340;background:#fff;padding:8mm}
        .hd{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2.5pt solid #4f46e5;padding-bottom:6pt;margin-bottom:10pt}
        .hd-r{font-size:9pt;line-height:1.9;color:#4b4f6b}
        .hd-r b{color:#1f2340}
        .ttl{text-align:center;flex:1}
        .ttl h1{font-size:17pt;font-weight:900;color:#4f46e5;letter-spacing:-.3pt}
        .ttl span{font-size:8.5pt;color:#7a7f9e}
        table{width:100%;border-collapse:collapse;table-layout:fixed}
        th,td{border:.8pt solid #d3d5e0}
        th{background:#eef0fb;color:#3b3f63;font-size:9.5pt;font-weight:900;padding:5pt 3pt}
        th.d{width:18%}
        td{height:44pt;vertical-align:middle;text-align:center;padding:3pt}
        td.p{background:#f5f6fb;width:10%}
        td.p b{display:block;font-size:11pt;font-weight:900;color:#1f2340}
        td.p span{display:block;font-size:7pt;color:#7a7f9e;line-height:1.5;margin-top:1pt}
        td.f{border-right:3pt solid var(--c)}
        td.f .n{display:block;font-size:10pt;font-weight:800;color:var(--c)}
        td.f .s{display:block;font-size:7.5pt;color:#7a7f9e;margin-top:1pt}
        td.e{background:#fbfbfe}
        .lgb{display:flex;flex-wrap:wrap;gap:4pt 12pt;margin-top:9pt;padding-top:7pt;border-top:.8pt solid #e7e8f0}
        .lg{display:inline-flex;align-items:center;gap:4pt;font-size:8.5pt;font-weight:700;color:#4b4f6b}
        .lg i{width:8pt;height:8pt;border-radius:2pt;display:inline-block}
        .ft{display:flex;justify-content:space-between;margin-top:8pt;font-size:7.5pt;color:#a3a7bf}
        .np{text-align:center;margin-top:14pt}
        .np button{padding:9pt 26pt;background:#4f46e5;color:#fff;border:none;border-radius:7pt;font:inherit;font-size:10pt;font-weight:800;cursor:pointer}
        @media print{.np{display:none}body{padding:0}}
      </style></head><body>
      <div class="hd">
        <div class="hd-r">
          <div><b>${_esc(teacher.school || '—')}</b></div>
          <div>${_esc(teacher.name || '—')}${main ? ' · ' + _esc(main) : ''}</div>
        </div>
        <div class="ttl">
          <h1>جدول الحصص الأسبوعي</h1>
          <span>${classes.length} فصول · ${sched.length} حصة أسبوعياً</span>
        </div>
        <div class="hd-r" style="text-align:left"><div>التاريخ</div><div><b>${today}</b></div></div>
      </div>
      <table>
        <thead><tr><th style="width:10%">الحصة</th>${hdr}</tr></thead>
        <tbody>${rows}</tbody>
      </table>
      ${classes.length ? `<div class="lgb">${legend}</div>` : ''}
      <div class="ft"><span>منصة المعلم — نظام إدارة الفصل الدراسي</span><span>طُبع في ${today}</span></div>
      <div class="np"><button onclick="window.print()">🖨 طباعة الجدول</button></div>
    </body></html>`;
      ...
      units: [
        {
          ...
          lessons: [
            ...
            { n: 2, title: 'سطح المكتب', from: 20, to: 32, presentation: 'presentations/g4/u1-l2.html', topics: [
            ...
            { n: 3, title: 'إعدادات جهاز الحاسب', from: 33, to: 38, presentation: 'presentations/g4/u1-l3.html', topics: [
        ...
        { n: 2, title: 'العمل على النص', part: 1,
          ...
          lessons: [
            { n: 1, title: 'لوحة المفاتيح', from: 44, to: 52, presentation: 'presentations/g4/u2-l1.html', topics: [
            ...
            { n: 2, title: 'تحرير النص', from: 53, to: 65, presentation: 'presentations/g4/u2-l2.html', topics: [
            ...
            { n: 3, title: 'تنسيق النص', from: 66, to: 73, presentation: 'presentations/g4/u2-l3.html', topics: [
              ['حان وقت التنسيق',66],['إنشاء العنوان',67],['البحث عن النص',69],['التراجع عن إجراء',70],
              ['حفظ عملك',70],['التكبير والتصغير',70],['لنطبق معاً',72,1]
            ]},
            { n: 4, title: 'تنسيق الفقرة', from: 74, to: 85, presentation: 'presentations/g4/u2-l4.html', topics: [
              ['محاذاة النص',74],['الحدود والتظليل',75],['التعداد النقطي والترقيم',77],['إدراج الرموز',78],
              ['لنطبق معاً',79,1]
            ]}
          ],
          project: 83,
          review: 'presentations/g4/u2-review.html',
        ...
        { n: 1, title: 'عرضي التقديمي', part: 2,
          goals: [
            'إنشاء عرض تقديمي حول موضوع معين.',
            'استخدام النصوص والصور للتعبير عن أفكارك.',
        ...
        { n: 2, title: 'العمل على الأرقام', part: 2,
          goals: [
        ...
        { n: 3, title: 'استخدام الوسائط المتعددة', part: 2,
          goals: [
            'استخدام أدوات ويندوز والميكروفون للتسجيلات الصوتية.',
          ...
          terms: [['تعديل','Adjustment'],['قص','Crop'],['تأثير','Effect'],['تعرض','Exposure'],
                  ...
                  ['إزالة العين الحمراء','Red Eye Removal'],['متابعة','Resume'],['تدوير','Rotate'],
                  ['مقطع صوتي','Sound Clip'],['مقطع فيديو','Video Clip']]
        },
        { n: 4, title: 'مقدمة في علم الروبوت', part: 2,
          goals: [
            'المقصود بالروبوت.',
            'مكونات روبوت ليجومايند ستورم.',
          ...
          terms: [['المقطع البرمجي','Code'],['مستشعر الألوان','Color sensor'],
                  ...
                  ['التوجيه','Steer'],['مستشعر اللمس','Touch sensor'],
                  ['مستشعر الموجات فوق الصوتية','Ultrasonic sensor'],['روبوت افتراضي','Virtual robot']]
        }
      ]
    },
    {
      ...
      units: [
        { n: 1, title: 'تعلم الأساسيات', part: 1,
          ...
          lessons: [
            { n: 1, title: 'أجهزة الحاسب', from: 15, to: 24, presentation: 'presentations/g5/u1-l1.html', topics: [
              ['ما الحاسب؟',15],['أنواع أجهزة الحاسب',15],['أدوات الحاسب التفاعلية',17],['لنطبق معًا',20,1]
            ]},
            { n: 2, title: 'أجزاء الحاسب', from: 25, to: 38, presentation: 'presentations/g5/u1-l2.html', topics: [
              ['أجزاء الحاسب الرئيسة',25],['أجهزة التخزين الخارجية',27],['أجهزة الإدخال',28],
              ['أجهزة الإخراج',32],['لنطبق معًا',34,1]
            ]},
            { n: 3, title: 'الملفات والمجلدات', from: 39, to: 54, presentation: 'presentations/g5/u1-l3.html', topics: [
              ['حجم الملف',39],['إنشاء اختصار',39],['الملفات والمجلدات المضغوطة',41],
              ['طريقة عرض قائمة الملفات',44],['سلة المحذوفات',46],['لنطبق معًا',47,1]
            ]}
          ],
          project: 55,
          review: 'presentations/g5/u1-review.html',
        ...
        { n: 2, title: 'التعامل مع المستندات', part: 1,
          ...
          lessons: [
            { n: 1, title: 'الصور والرسومات', from: 62, to: 68, presentation: 'presentations/g5/u2-l1.html', topics: [
              ['إدراج صورة من الإنترنت',62],['إدراج صورة من جهاز الحاسب',63],['تعديل الصور',63],
              ['إدراج الأشكال',65],['لنطبق معًا',66,1]
            ]},
            { n: 2, title: 'التنسيق المتقدم', from: 69, to: 80, presentation: 'presentations/g5/u2-l2.html', topics: [
            ...
            { n: 3, title: 'إدراج الرسومات التوضيحية', from: 81, to: 89, presentation: 'presentations/g5/u2-l3.html', topics: [
              ['إدراج الرسوم التوضيحية',81],['تنسيق الرسوم التوضيحية',83],['أنواع الرسوم التوضيحية SmartArt',84],
              ['لنطبق معًا',87,1]
            ]},
            { n: 4, title: 'التدقيق والطباعة', from: 90, to: 96, presentation: 'presentations/g5/u2-l4.html', topics: [
              ['التدقيق والتحقق من الأخطاء',90],['البحث عن المرادفات',91],['الطباعة',92],['لنطبق معًا',94,1]
            ]}
          ],
          project: 97,
          review: 'presentations/g5/u2-review.html',
        ...
        { n: 3, title: 'الوسائط المتعددة', part: 1,
          ...
          lessons: [
            { n: 1, title: 'استخدام أجهزة الالتقاط وتحرير مقاطع الصوت', from: 103, to: 131, presentation: 'presentations/g5/u3-l1.html', topics: [
            ...
            { n: 2, title: 'البحث عن الوسائط المتعددة وإنشاء وتحرير مقاطع الفيديو', from: 132, to: 154, presentation: 'presentations/g5/u3-l2.html', topics: [
          ...
          project: 155,
          review: 'presentations/g5/u3-review.html',
        ...
        { n: 4, title: 'البرمجة والتفاعل في سكراتش', part: 1,
          ...
          lessons: [
            { n: 1, title: 'كيفية تصميم برنامج', from: 162, to: 169, presentation: 'presentations/g5/u4-l1.html', topics: [
              ['ما البرنامج؟',162],['اتباع التعليمات',163],['المخطط الانسيابي',164],
              ['خطوات إنشاء برنامج',165],['لنطبق معًا',167,1]
            ]},
            { n: 2, title: 'الكائنات في سكراتش', from: 170, to: 181, presentation: 'presentations/g5/u4-l2.html', topics: [
            ...
            { n: 3, title: 'المعاملات الشرطية', from: 182, to: 192, presentation: 'presentations/g5/u4-l3.html', topics: [
            ...
            { n: 4, title: 'الحركة في سكراتش', from: 193, to: 206, presentation: 'presentations/g5/u4-l4.html', topics: [
            ...
            { n: 5, title: 'رسائل البث', from: 207, to: 216, presentation: 'presentations/g5/u4-l5.html', topics: [
              ['الأحداث في سكراتش',207],['ما هو الرسم المتحرك؟',208],['مشروع "رسوم متحركة في الفضاء"',209],
              ['لنطبق معًا',214,1]
            ]},
            { n: 6, title: 'الاستشعار', from: 217, to: 224, presentation: 'presentations/g5/u4-l6.html', topics: [
              ['لبنات الاستشعار',217],['ملامسة اللون',218],['التحقق من ملامسة مؤشر الفأرة',221],['لنطبق معًا',223,1]
            ]}
          ],
          project: 225,
          review: 'presentations/g5/u4-review.html',
        ...
        { n: 1, title: 'أدوات البحث والاتصال ومشاركة الملفات', part: 2,
          goals: [
          ...
          lessons: [
            { n: 1, title: 'الإنترنت و الشبكة العنكبوتية', from: 245, to: 254, presentation: null, topics: [
              ['شبكات الحاسب',245],['الفرق بين الإنترنت والشبكة العنكبوتية العالمية',245],
              ['محرّكات البحث',246],['تخصيص البحث',248],['إضافة المواقع إلى المفضلة',249],
              ['استخدامات أخرى لمحرك البحث',250],['لنطبق معاً',252,1]
            ]},
            { n: 2, title: 'الإنترنت وأدوات التواصل', from: 255, to: 268, presentation: null, topics: [
              ['البريد الإلكتروني',255],['المحادثة',256],['تخصيص المجموعة',263],
              ['المكالمات الصوتية ومكالمات الفيديو',264],['لنطبق معاً',266,1]
            ]},
            { n: 3, title: 'مشاركة الملفات', from: 269, to: 280, presentation: null, topics: [
              ['الدخول على ون درايف',269],['مشاركة الملفات على ون درايف',270],
              ['إضافة الملفات على ون درايف',272],['عرض الصور',273],
              ['كن آمناً عبر الإنترنت',274],['لنطبق معاً',276,1]
            ]}
          ],
          project: 281,
          terms: [['حساب','Account'],['المحادثة','Chat'],['السحابة','Cloud'],['تنزيل','Download'],
                  ['الرموز التعبيرية','Emoticon'],['فورية','Instant'],['الإنترنت','Internet'],
                  ['شبكة المنطقة المحلية','LAN'],['رسالة','Message'],['شبكة','Network'],
                  ['ون درايف','OneDrive'],['كلمة مرور','Password'],['مجلد عام','Public Folder'],
                  ['محرك بحث','Search Engine'],['مشاركة','Share'],['تحميل','Upload'],
                  ['اسم مستخدم','Username'],['مكالمة فيديو','Video Call'],['مكالمة صوتية','Voice Call'],
                  ['شبكة المنطقة الواسعة','WAN']]
        },
        { n: 2, title: 'جداول البيانات', part: 2,
          goals: [
            ...
            'إجراء العمليات الحسابية باستخدام الدوال SUM وAVERAGE وMAX وMIN.',
            'استخدام ميزة التعبئة التلقائية.',
            'تنسيق الأرقام العشرية.'
          ],
          lessons: [
            { n: 1, title: 'الصفوف والأعمدة', from: 287, to: 302, presentation: null, topics: [
              ['تغيير عرض العمود',288],['تغيير ارتفاع الصف',290],['دمج الخلايا',291],
              ['التفاف النص',292],['إدراج الصفوف والأعمدة',293],['حذف الصفوف والأعمدة',296],
              ['محاذاة النصوص والأرقام',297],['تغيير زاوية اتجاه النص في الخلية',298],
              ['لنطبق معاً',299,1]
            ]},
            { n: 2, title: 'العمليات الحسابية', from: 303, to: 314, presentation: null, topics: [
              ['الدوال',303],['دالة المجموع',304],['ميزة التعبئة التلقائية',305],
              ['دالة المتوسط',306],['تنسيق الأرقام العشرية',308],
              ['دالة الحد الأدنى لأصغر قيمة والحد الأقصى لأكبر قيمة',309],['لنطبق معاً',312,1]
            ]}
          ],
          project: 315,
          terms: [['المحاذاة','Alignment'],['تعبئة تلقائية','Auto Fill'],['احتواء تلقائي','Autofit'],
                  ['دالة المتوسط','Average'],['عرض العمود','Column Width'],['أرقام عشرية','Decimals'],
                  ['حذف','Delete'],['دوال','Functions'],['إدراج','Insert'],['الحد الأقصى','Max'],
                  ['دمج وتوسيط','Merge & Center'],['الحد الأدنى','Min'],['زاوية اتجاه النص','Orientation'],
                  ['ارتفاع الصف','Row Height'],['المجموع','Sum'],['التفاف النص','Wrap Text']]
        },
        { n: 3, title: 'وسائل التواصل الاجتماعي', part: 2,
          goals: [
          ...
          lessons: [
            { n: 1, title: 'وسائل التواصل الاجتماعي', from: 320, to: 325, presentation: null, topics: [
              ['وسائل التواصل الاجتماعي',320],['قواعد الأمان عند استخدام الإنترنت',321],
              ['حماية الحاسب',322],['لنطبق معاً',323,1]
            ]},
            { n: 2, title: 'التدوين', from: 326, to: 346, presentation: null, topics: [
              ['ما المدوّنة؟',326],['ضوابط إنشاء المدونة',326],['قواعد كتابة التدوينات',327],
              ['كيفية إنشاء مدونتك الخاصة باستخدام تطبيق بلوقر',328],['تحرير ملفك الشخصي',332],
              ['النشر على مدونتك',334],['إدراج الصور في مدونتك',336],
              ['إدراج مقاطع الفيديو في مدونتك',338],['معاينة ونشر مدونتك',340],
              ['تحديث تدوينتك',342],['نشر تعليق',343],['لنطبق معاً',344,1]
            ]},
            { n: 3, title: 'الملكية الفكرية', from: 347, to: 351, presentation: null, topics: [
              ['ما المقصود بالملكية الفكرية؟',347],['قانون الملكية الفكرية',347],
              ['مواد ذات حقوق محفوظة',347],['تراخيص المشاع الإبداعي',347],
              ['القرصنة عبر الإنترنت',348],['لنطبق معاً',349,1]
            ]}
          ],
          project: 352,
          terms: [['مُدوّنة','Blog'],['تعليق','Comment'],['مُتابع','Follower'],
                  ['مُلكية فكرية','Intellectual Property'],['إنستغرام','Instagram'],
                  ['ألعاب عبر الإنترنت','Online Gaming'],['معلومات شخصية','Personal Information'],
                  ['قَرصنة','Piracy'],['مُشاركة','Post'],['مجال عام / مُلك مشاع','Public Domain'],
                  ['السلامة','Safety'],['شبكة تواصل اجتماعي','Social Network']]
        },
        { n: 4, title: 'برمجة الروبوت', part: 2,
          goals: [
          ...
          lessons: [
            { n: 1, title: 'الروبوتات في حياتنا اليومية', from: 359, to: 367, presentation: null, topics: [
              ['ماهية الروبوت',359],['أنواع الروبوتات',359],['استخدامات الروبوتات',360],
              ['تأثيرات استخدام الروبوتات',361],['برمجة الروبوت ليتحرك ويرسم دائرة',362],
              ['لنطبق معاً',365,1]
            ]},
            { n: 2, title: 'استخدام التكرارات', from: 368, to: 382, presentation: null, topics: [
              ['برمجة الروبوت ليتحرك ويرسم مثلث',368],['برمجة الروبوت ليتحرك ويرسم مستطيل',373],
              ['لنطبق معاً',379,1]
            ]},
            { n: 3, title: 'رسم مكعب', from: 383, to: 408, presentation: null, topics: [
              ['برمجة الروبوت ليتحرك ويرسم المضلع السداسي',384],['برمجة الروبوت لينعطف',387],
              ['برمجة الروبوت لإضافة مؤثر صوتي',388],['عرض رسالة على شاشة عرض الروبوت',390],
              ['برمجة الروبوت ليتحرك ويرسم المعين',393],['لنطبق معاً',403,1]
            ]}
          ],
          project: 409,
          terms: [['لبنة','Block'],['لبنة مسح العرض','Clear Display Block'],['فئة التحكم','Control Category'],
                  ['الهندسة','Engineering'],['الروبوتات الثابتة','Fixed Robots'],
                  ['الأشكال الهندسية','Geometric Shapes'],['المضلع السداسي','Hexagon'],
                  ['مليلي ثانية','Milliseconds'],['الروبوتات المتنقلة','Mobile Robots'],
                  ['المضلع الثماني','Octagon'],['المضلع الخماسي','Pentagon'],['المضلع','Polygon'],
                  ["عرض الروبوت",'Robot\'s View'],['نافذة المحاكاة','Simulation View']]
        }
      ]
    },
    {
      ...
      units: [
        ...
        { n: 1, title: 'التصميم المتقدم للمستندات', part: 2,
          goals: [
            'إنشاء جدول وتنسيقه باستخدام الأنماط.',
            'إضافة وحذف الصفوف والأعمدة من الجدول.',
            'ضبط حجم الجدول.',
            'تحرير الجداول.',
            'البحث عن الكلمات واستبدالها.',
            'تنسيق النص في عدّة أعمدة.',
          ...
          lessons: [
            { n: 1, title: 'إنشاء الجداول وتنسيقها', from: 211, to: 218, presentation: null, topics: [
              ['إنشاء الجداول',211],['تنسيق الجدول',213],['لنطبق معًا',216,1]
            ]},
            { n: 2, title: 'تحرير الجداول', from: 219, to: 227, presentation: null, topics: [
              ['إضافة الصفوف والأعمدة',219],['ضبط حجم الجدول',221],['إجراء التحديدات',222],
              ['محاذاة النص',223],['البحث عن الكلمات واستبدالها',224],['لنطبق معًا',225,1]
            ]},
            { n: 3, title: 'التنسيق المتقدم', from: 228, to: 245, presentation: null, topics: [
              ['إدراج النص في أعمدة',228],['المسافة البادئة للنص باستخدام المسطرة',230],
              ['الرؤوس والتذييلات',232],['إضافة الرموز',234],['تطبيق نمط',236],
              ['فاصل صفحة',237],['طرق عرض المستند',238],['لإضافة صفحة غلاف',241],
              ['لنطبق معًا',242,1]
            ]}
          ],
          project: 246,
          terms: [['محاذاة','Align'],['مسافة بادئة','Indent'],['احتواء تلقائي','AutoFit'],
                  ['فاصل صفحة','Page Break'],['حَد','Border'],['وضع القراءة','Read Mode'],
                  ['خلية','Cell'],['صف','Row'],['عمود','Column'],['تخطيط الطباعة','Print Layout'],
                  ['اتجاه','Direction'],['تظليل','Shading'],['تذييل','Footer'],['نمط','Style'],
                  ['رأس صفحة','Header']]
        },
        { n: 2, title: 'تصميم المواقع الإلكترونية', part: 2,
          goals: [
            'ماهية الشبكة العنكبوتية، والمواقع الإلكترونية، والصفحات الإلكترونية والعلاقة بينهم.',
          ...
          lessons: [
            { n: 1, title: 'تصميم صفحة إلكترونية', from: 252, to: 271, presentation: null, topics: [
              ['الشبكة الإلكترونية',252],['الموقع الإلكتروني',253],['الصفحة الإلكترونية',253],
              ['إنشاء موقع على شبكة الإنترنت باستخدام أداة جوجل',254],['التعامل مع النصوص',258],
              ['إضافة الصور',266],['لنطبق معًا',269,1]
            ]},
            { n: 2, title: 'إضافة الصفحات', from: 272, to: 282, presentation: null, topics: [
              ['أهمية تعدد الصفحات في الموقع الإلكتروني',272],['إنشاء الصفحات الإلكترونية',273],
              ['تخطيط الصفحة',275],['تنظيم صفحاتك',279],['إضافة الارتباطات التشعبية',280],
              ['لنطبق معًا',281,1]
            ]},
            { n: 3, title: 'نشر الموقع الإلكتروني', from: 283, to: 291, presentation: null, topics: [
              ['إضافة أيقونات وسائل التواصل الاجتماعي',283],['معاينة التغييرات',287],
              ['نشر الموقع الإلكتروني ومشاركته عبر الإنترنت',288],['لنطبق معًا',290,1]
            ]}
          ],
          project: 292,
          terms: [['خلفية','Background'],['مجموعة المحتوى','Content Block'],['عنصر','Element'],
                  ['أداة مواقع جوجل','Google Sites'],['صفحة رئيسة','Home Page'],
                  ['لغة ترميز النص التشعبي','HTML'],['ارتباط تشعبي','Hyperlink'],['صورة','Image'],
                  ['شبكة إلكترونية','Network'],['تخطيط الصفحة','Page layout'],['نشر','Publish'],
                  ['وسائل التواصل الاجتماعي','Social media'],['نص','Text'],
                  ['صفحة إلكترونية','Webpage'],['موقع إلكتروني','Website']]
        },
        { n: 3, title: 'تصميم ألعاب جهاز الحاسب', part: 2,
          goals: [
          ...
          lessons: [
            { n: 1, title: 'تخطيط وتصميم ألعاب جهاز الحاسب', from: 296, to: 313, presentation: null, topics: [
              ['المكونات الرئيسة للألعاب',296],['خطوات تصميم اللعبة',297],['وصف اللعبة',298],
              ['إنشاء اللعبة باستخدام مختبر لعبة كودو',300],['إضافة الكائن الرئيس',304],
              ['تحريك الكاميرا',305],['إضافة التضاريس',306],['حفظ اللعبة',310],
              ['تحميل اللعبة',311],['لنطبق معًا',312,1]
            ]},
            { n: 2, title: 'برمجة ألعاب جهاز الحاسب', from: 314, to: 326, presentation: null, topics: [
              ['برمجة الكائن',314],['برمجة نظام الفوز بالنقاط',320],['اختبار اللعبة',324],
              ['لنطبق معًا',325,1]
            ]}
          ],
          project: 327,
          terms: [['الشخصية','Character'],['عملية التصميم','Design Process'],['لعبة','Game'],
                  ['كائنات','Objects'],['نقاط','Score'],['تضاريس','Terrain'],['عالم','World']]
        },
        { n: 4, title: 'المستشعرات في علم الروبوت', part: 2,
          goals: [
          ...
          lessons: [
            { n: 1, title: 'مستشعرات الروبوت', from: 334, to: 347, presentation: null, topics: [
              ['مستشعرات الروبوت',334],['لبنات فئة المستشعرات',335],['مستشعر الموجات فوق الصوتية',335],
              ['مستشعر الألوان',341],['اختبار المقطع البرمجي وتشخيص الأخطاء',344],['لنطبق معًا',346,1]
            ]},
            { n: 2, title: 'اتخاذ القرارات', from: 348, to: 362, presentation: null, topics: [
              ['برمجة الروبوت لاتخاذ القرارات',348],['لنطبق معًا',361,1]
            ]},
            { n: 3, title: 'إنشاء الخرائط', from: 363, to: 377, presentation: null, topics: [
              ['إضافة العوائق وتلوين المساحات',363],['إنشاء الخرائط',368],['لنطبق معًا',375,1]
            ]}
          ],
          project: 378,
          terms: [['وضع الألوان','Colour Mode'],['لبنة ملتقط الألوان','Colour Picker Block'],
                  ['مستشعر اللون','Colour Sensor'],['لبنة المقارنة','Comparison Block'],
                  ['شرط','Condition'],['وضع التصحيح','Debugging Procedure'],
                  ['وضع الإضاءة','Light Mode'],['معامل منطقي','Logical Operator'],
                  ['لبنة الرقم','Number Block'],['عائق','Obstacle'],
                  ['عرض بيانات المستشعرات','Sensors Data View'],
